<div class="wrap">
	...
	<form action="settings.php?page=multisite-theme-manager.php" method="post" enctype="multipart/form-data">
		...
		<h3><?php _e('General Settings', 'wmd_prettythemes') ?></h3>

		<table class="form-table">
			<tr valign="top">
				<th scope="row">
					<label for="wmd_prettythemes_options[setup_mode]"><?php _e('Setup Mode', 'wmd_prettythemes') ?></label>
				</th>
				...
				<td>
					...
					<p class="description"<?php echo ($this->options['setup_mode'] == 1) ? ' style="color:red;"' : ''; ?>><?php _e('When set to "Setup Mode Enabled", the Multisite Theme Manager theme page will be visible only on the main site. This mode is useful for configuring theme details before enabling the Multisite Theme Manager features on all sites in the network.', 'wmd_prettythemes') ?></p>
				</td>
			</tr>

			<tr valign="top">
				<th scope="row">
					<label for="wmd_prettythemes_options[theme]"><?php _e('Select Theme For Theme Page', 'wmd_prettythemes') ?></label>
				</th>
				...
				<td>
					...
					<p class="description"><?php _e('Choose the theme that you want to use to display your theme page. You can add your own themes into "wp-content/upload/prettythemes/your-theme/" folder. (Tip: duplicate the default theme from "wp-content/plugins/pretty-themes/multisite-theme-manager-files/themes/" to get started.)', 'wmd_prettythemes') ?></p>
				</td>
			</tr>

		</table>

		<h3><?php _e('Theme and Appearance Settings', 'wmd_prettythemes'); ?></h3>

		<table class="form-table">
			<tr valign="top">
				<th scope="row">
					<label for="wmd_prettythemes_options[themes_options]"><?php _e('Select details that you want to show for themes', 'wmd_prettythemes') ?></label>
				</th>
				...
				<td>
					...
					<p class="description"><?php _e('Choose which links will be displayed for each theme', 'wmd_prettythemes') ?></p>
				</td>
			</tr>

			<tr valign="top">
				<th scope="row">
					<label for="wmd_prettythemes_options[themes_auto_screenshots_by_name]"><?php _e('Auto Load Screenshot With Correct Name', 'wmd_prettythemes') ?></label>
				</th>
		...
		<h3><?php _e('Labels', 'wmd_prettythemes') ?></h3>

		<table class="form-table">
			<tr valign="top">
				<th scope="row">
					<label for="wmd_prettythemes_options[themes_link_label]"><?php _e('Theme Page Title', 'wmd_prettythemes') ?></label>
				</th>
				...
				<td>
					...
					<p class="description"><?php _e('This is what you call the "Themes" menu item. Call it "Themes", "Design", "Page Look" or whatever you\'d like.', 'wmd_prettythemes') ?></p>
				</td>
			</tr>
			<tr valign="top">
				<th scope="row">
					<label for="wmd_prettythemes_options[themes_page_description]"><?php _e('Theme Page Description', 'wmd_prettythemes') ?></label>
				</th>
				...
				<td>
					...
					<p class="description"><?php _e('This text will be visible at the top of the themes page. Tell your users what you have to offer.', 'wmd_prettythemes') ?></p>
				</td>
			</tr>
			<tr valign="top">
				<th scope="row">
					<label for="wmd_prettythemes_options[themes_link_label]"><?php _e('Default Custom Link Label', 'wmd_prettythemes') ?></label>
				</th>
				...
				<td>
					...
					<p class="description"><?php _e('This will be default label for custom link that you can add for each theme when editing details (for example, a link to support documents for the theme). You can also overwrite label per theme.', 'wmd_prettythemes') ?></p>
				</td>
			</tr>
		</table>

		<p class="submit">
			<input type="submit" name="save_settings" class="button-primary" value="<?php _e('Save Changes', 'wmd_prettythemes') ?>" />
		</p>

		<h3><?php _e('Export and Import', 'wmd_prettythemes'); ?></h3>
		
		<?php $prettythemes_options_nonce = wp_create_nonce('wmd_prettythemes_options'); ?>
		<table class="form-table">
			<tr valign="top">
				<th scope="row">
					<label><?php _e('Export', 'wmd_prettythemes') ?></label>
				</th>
			...
			<tr valign="top">
				<th scope="row">
					<label for="wmd_prettythemes_options[themes_link_label]"><?php _e('Import:', 'wmd_prettythemes') ?></label>
				</th>

				<td>
					<input type="file" name="config_file" id="upload" size="25">
					<input type="submit" name="import_config" class="button" value="<?php _e('Upload file and import', 'wmd_prettythemes'); ?>"/>
					<p class="description"><?php _e('Choose an export file (correctly formatted XML file) to import data and settings. This action will replace any existing data and settings.', 'wmd_prettythemes') ?></p>
				</td>
			</tr>

			<tr valign="top">
				<th scope="row">
					<label><?php _e('Reset:', 'wmd_prettythemes') ?></label>
				</th>

				<td>
					<a onclick="return confirm('<?php _e('Are you sure?', 'wmd_prettythemes'); ?>')" href="<?php echo add_query_arg(array('prettythemes_action' => 'delete_custom_data', '_wpnonce' => $prettythemes_options_nonce)); ?>" class="button"><?php _e('Delete all custom theme data', 'wmd_prettythemes') ?></a>
					<a onclick="return confirm('<?php _e('Are you sure?', 'wmd_prettythemes'); ?>')" href="<?php echo add_query_arg(array('prettythemes_action' => 'reset_settings', '_wpnonce' => $prettythemes_options_nonce)); ?>" class="button"><?php _e('Reset all settings', 'wmd_prettythemes') ?></a>

					<p class="description">
						<?php _e('This action will permanently delete choosen data.', 'wmd_prettythemes') ?></br>
					</p>
				</td>
			</tr>
		</table>
